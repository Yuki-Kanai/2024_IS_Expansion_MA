...
def plotsr(args):
    ...
    from plotsr_func import setlogconfig, readbasecfg, readsyriout, readbedout, filterinput, validalign2fasta, selectchrom, selectregion, createribbon, drawax, pltchrom, pltsv, drawmarkers, readtrack, drawtracks, getfilehandler, definelogger
    ...
    ## Draw Axes
    ax = drawax(ax, chrgrps, chrlengths, V, S, cfg, ITX, minl=minl, maxl=maxl, chrname=CHRNAME, noxaxislabel=args.noxaxislabel, noyaxislabel=args.noyaxislabel)
    ...
    ## override xlim
    if args.xlim != -1:
        print(minl, maxl, args.xlim, ax.get_xlim())
        ax.set_xlim(minl, args.xlim)
...
def plotsr_main(cmd):
    ...
    plotting.add_argument('--legend', help='Do not plot legend', default=True, action='store_false')
    plotting.add_argument('--noyaxislabel', help='Do not plot y-axis label', default=False, action='store_true')
    plotting.add_argument('--noxaxislabel', help='Do not plot y-axis label', default=False, action='store_true')
    plotting.add_argument('--xlim', help='fix xlim size to', default=-1, type=int)
    ...
    other.add_argument('--log', help='Log-level', choices=['DEBUG', 'INFO', 'WARN'], default='INFO', type=str)
