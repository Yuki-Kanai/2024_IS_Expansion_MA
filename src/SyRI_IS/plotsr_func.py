...
def getfilehandler(fin, level):
    ...
    def filehandler():
        ...
        fhandle.setFormatter(logging.Formatter(logformat))
        return fhandle
    return filehandler
# END
...
def streamhandler():
    ...
    shandler.setFormatter(CustomFormatter())
    return shandler
# END
...
def drawax(ax, chrgrps, chrlengths, v, s, cfg, itx, minl=0, maxl=-1, chrname=None, noxaxislabel=False, noyaxislabel=False):
    import numpy as np
    from collections import deque
...
def pltsv(ax, alignments, chrs, v, chrgrps, chrlengths, indents, S, cfg, itx, maxl):
    ...
    for s in range(len(alignments)):
        ...
        # Read alignment annotation as provided by user
...
def drawmarkers(ax, b, v, chrlengths, indents, chrs, chrgrps, S, cfg, itx, minl=0, maxl=-1):
    ...
    for m in mdata:
        ...
        if not itx:
            ...
            if not v:
                if m.mt != '_':
                    m.tp = float(m.tp) # bug in plotsr when no input
